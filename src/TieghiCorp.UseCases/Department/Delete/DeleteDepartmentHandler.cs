    ...
    IQueryRepos<Core.Entities.Personnel> personnelQuery) : IRequestHandler<DeleteDepartmentRequest, Result>
{
    ...
    {
        if (!await _departmentQuery.FindByKeyAsync(d => d.Id == request.Id, cancellationToken))
        {
            return Result.Failure(HttpError.NotFound("Department", request.Id));
        }

        if (await _personnelQuery.FindByKeyAsync(d => d.DepartmentId == request.Id, cancellationToken))
        {
